#' Create a new NONxbrlQueryPiece
#'
#' @description
#' NONxbrlQueryPiece Class
#'
#' @docType class
#' @title NONxbrlQueryPiece
#' @description NONxbrlQueryPiece Class
...
#' @export
NONxbrlQueryPiece <- R6::R6Class(
  "NONxbrlQueryPiece",
  public = list(
    `parameter` = NULL,
    `Operator` = NULL,
    `value` = NULL,
    #' Initialize a new NONxbrlQueryPiece class.
    #'
    #' @description
    #' Initialize a new NONxbrlQueryPiece class.
    #'
    ...
    #' To JSON String
    #'
    #' @return NONxbrlQueryPiece in JSON format
    #' @export
    toJSON = function() {
      NONxbrlQueryPieceObject <- list()
      if (!is.null(self$`parameter`)) {
        NONxbrlQueryPieceObject[["parameter"]] <-
          self$`parameter`
      }
      if (!is.null(self$`Operator`)) {
        NONxbrlQueryPieceObject[["Operator"]] <-
          self$`Operator`
      }
      if (!is.null(self$`value`)) {
        NONxbrlQueryPieceObject[["value"]] <-
          self$`value`
      }
      NONxbrlQueryPieceObject
    },
    #' Deserialize JSON string into an instance of NONxbrlQueryPiece
    #'
    #' @description
    #' Deserialize JSON string into an instance of NONxbrlQueryPiece
    #'
    #' @param input_json the JSON input
    #' @return the instance of NONxbrlQueryPiece
    #' @export
    ...
    #' Validate JSON input with respect to NONxbrlQueryPiece
    #'
    #' @description
    #' Validate JSON input with respect to NONxbrlQueryPiece and throw an exception if invalid
    #'
    ...
    #' To string (JSON format)
    #'
    #' @return String representation of NONxbrlQueryPiece
    #' @export
    toString = function() {
      self$toJSONString()
    },
...
# NONxbrlQueryPiece$unlock()
#
## Below is an example to define the print function
# NONxbrlQueryPiece$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# NONxbrlQueryPiece$lock()
